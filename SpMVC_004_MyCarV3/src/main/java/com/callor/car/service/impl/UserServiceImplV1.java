...
public class UserServiceImplV1 implements UserService {
	...
	/*
	 * 회원가입 절차 1.user table 에 현재 입력된 user 가 있는가? true : 현재 추가할 사용자의 ROLE = "user"
	...
	/*
	 * login 을 요청받았을때 username 이 없는경우, password가 틀린경우, username 도 있고, password도 맞는데
	 * 아직 로그인이 승인 되지 않은 경우
	 * 
	 */
	@Override
	public UserDto login(UserDto userDto) throws Exception {
		...
		UserDto resultDto = userDao.findById(userDto.getUsername());
			// username 이 없는경우
			/*
			 * throw : 나를 호출한 곳으로 Exception 을 되돌린다. Exception을 던진다. Toss 한다 new Exception()
			 * : 새로운 Exception 강제로 Exception 을 만들어서 Controller 에게 보내버리기
			 */
		if (resultDto == null) {
